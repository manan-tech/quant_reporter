"""
Portfolio Optimization Report — Sample with Advanced Strategies
================================================================
This script demonstrates portfolio optimization using quant_reporter,
including the new advanced optimization methods.
...
1. **8 Portfolio Strategies Compared**:
   ...
   - Sector Balanced (with sector constraints)
...
Configuration:
    ...
    - Sector caps (max allocation) and minimums
...
def main():
    ...
    report_path = os.path.join(REPORTS_DIR, 'Optimization_Report.html')

    logger = logging.getLogger(__name__)
    logger.info("=" * 70)
    logger.info("  PORTFOLIO OPTIMIZATION REPORT")
    logger.info("=" * 70)
    logger.info("  Tickers     : %d", len(portfolio))
    logger.info("  Sectors     : %d", len(set(sector_map.values())))
    logger.info("  Strategies  : 9 (4 traditional + 1 sector + 4 advanced)")
    logger.info("  Period      : 2020-01-01 → 2023-12-31")
    logger.info("  Benchmark   : %s", benchmark)
    logger.info("  Output      : %s", report_path)
    logger.info("=" * 70)
    logger.info("")
    logger.info("  Traditional Strategies:")
    logger.info("    • Equal Weight (Baseline)")
    logger.info("    • Minimum Volatility")
    logger.info("    • Balanced (40%% Cap)")
    logger.info("    • Max Sharpe (Unconstrained)")
    logger.info("    • Sector Balanced (with caps/mins)")
    logger.info("")
    ...
    try:
        ...
        logger.info("")
        logger.info("📊  Open the report to compare all 9 strategies!")
        logger.info("    - 4 Traditional MPT strategies")
        logger.info("    - 1 Sector-constrained strategy")
        logger.info("    - 4 Advanced risk-based strategies")
